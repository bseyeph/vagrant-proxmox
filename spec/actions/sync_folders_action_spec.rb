...
module VagrantPlugins::Proxmox
  describe Action::SyncFolders do
    ...
    let(:env) do
      { machine: environment.machine(environment.primary_machine_name, :proxmox), ui: double('ui').as_null_object }
    end
    ...
    describe '#call' do
      before do
        allow(env[:machine]).to receive(:ssh_info) {
                                  { host: '127.0.0.1', port: 22, username: 'vagrant', private_key_path: ['key'] }
                                }
        allow(env[:machine].communicate).to receive :sudo
        allow(Vagrant::Util::Subprocess).to receive_messages execute: Vagrant::Util::Subprocess::Result.new(0, nil,
                                                                                                            nil)
      end
      ...
      it 'should raise an error if the rsync fails' do
        allow(Vagrant::Util::Subprocess).to receive_messages execute: Vagrant::Util::Subprocess::Result.new(1, nil,
                                                                                                            nil)
