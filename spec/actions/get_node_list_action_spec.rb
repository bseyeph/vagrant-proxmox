...
module VagrantPlugins::Proxmox
  describe Action::GetNodeList do
    ...
    let(:env) do
      { machine: environment.machine(environment.primary_machine_name, :proxmox), proxmox_connection: connection }
    end
    ...
    describe '#call' do
      ...
      it 'should store the node list in env[:proxmox_nodes]' do
        expect(connection).to receive(:get_node_list).and_return %w[node1 node2]
        action.call env
        expect(env[:proxmox_nodes]).to eq(%w[node1 node2])
      end
