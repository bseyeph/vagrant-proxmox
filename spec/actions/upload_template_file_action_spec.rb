  ...
  describe Action::UploadTemplateFile do
    ...
    context 'with a specified template file' do
      it 'should upload the template file into the local storage of the selected node' do
        expect(connection).to receive(:upload_file).with(template_file, content_type: 'vztmpl', node: node,
                                                                        storage: 'local', replace: replace_template_file)
        action.call env
      end
    end
    ...
    context 'the template file should be replaced' do
      ...
      it 'should delete the template file on the server' do
        expect(connection).to receive(:delete_file).with(filename: template_file, content_type: 'vztmpl', node: node,
                                                         storage: 'local')
        action.call env
      end
    end
