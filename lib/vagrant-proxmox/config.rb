  ...
  module Proxmox
    class Config < Vagrant.plugin('2', :config)
      ...
      def validate(_machine)
        ...
        if @vm_type == :openvz && !(@openvz_os_template || @openvz_template_file)
          errors << I18n.t('vagrant_proxmox.errors.no_openvz_os_template_or_openvz_template_file_specified_for_type_openvz')
        end
        if @vm_type == :qemu
          if @qemu_template
          else
            errors << I18n.t('vagrant_proxmox.errors.no_qemu_os_specified_for_vm_type_qemu') unless @qemu_os
            unless @qemu_iso || @qemu_iso_file
              errors << I18n.t('vagrant_proxmox.errors.no_qemu_iso_or_qemu_iso_file_specified_for_vm_type_qemu')
            end
            unless @qemu_disk_size
              errors << I18n.t('vagrant_proxmox.errors.no_qemu_disk_size_specified_for_vm_type_qemu')
            end
          end
        end
