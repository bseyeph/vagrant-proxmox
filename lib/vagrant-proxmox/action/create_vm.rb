    ...
    module Action
      ...
      class CreateVm < ProxmoxAction
        ...
        def create_params_qemu(config, env, vm_id)
          ...
          if get_machine_macaddress(env)
            network = "#{config.qemu_nic_model}=#{get_machine_macaddress(env)},bridge=#{config.qemu_bridge}"
          end
          { vmid: vm_id,
        ...
        def create_params_lxc(config, env, vm_id)
          { vmid: vm_id,
            ...
            if get_machine_ip_address(env)
              params[:net0] =
                "name=#{get_machine_interface_name(env)},ip=#{get_machine_ip_address(env)}/24,gw=#{get_machine_gw_ip(env)},bridge=#{get_machine_bridge_name(env)}"
            end
          end
        end
      end
