      ...
      class ConfigClone < ProxmoxAction
        ...
        def call(env)
          ...
          begin
            ...
            @guest_port = format('22%03d', vm_id.to_i).to_s
        ...
        def create_params_qemu(provider_config, env, vm_id, template_config)
          ...
          device0 = [
            provider_config.qemu_nic_model.to_s,
            'netdev=net0',
            'bus=pci.0',
          ...
          vm_config.networks.each do |type, options|
            next unless type.match(/^p.*_network$/)
          ...
          if provider_config.vm_memory && (provider_config.vm_memory < template_config[:balloon])
            params[:balloon] =
              provider_config.vm_memory.to_s.to_i
          end
