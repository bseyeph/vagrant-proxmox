...
module VagrantPlugins
  module Proxmox
    class Connection
      ...
      attr_reader :api_url, :ticket, :csrf_token
      attr_accessor :vm_id_range, :task_timeout, :task_status_check_interval, :imgcopy_timeout
      ...
      def login(username: required('username'), password: required('password'))
        ...
        raise ApiError::ConnectionError, e.message
      end
      ...
      def get_qemu_template_id(template)
        response = get '/cluster/resources?type=vm'
        found_ids = response[:data].select do |vm|
                      vm[:type] == 'qemu'
                    end.select { |vm| vm[:template] == 1 }.select { |vm| vm[:name] == template }.map { |vm| vm[:vmid] }
      ...
      def get_vm_info(vm_id)
        response = get '/cluster/resources?type=vm'
        response[:data]
          .select { |m| m[:id] =~ %r{^[a-z]*/#{vm_id}$} }
          .map { |m| { id: vm_id, type: %r{^(.*)/(.*)$}.match(m[:id])[1], node: m[:node] } }
          .first
      end
